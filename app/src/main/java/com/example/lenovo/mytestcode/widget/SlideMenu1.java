 ...
 */
public class SlideMenu1 extends FrameLayout {
  ...
  private int closeDragRange;
  private int openDragRange;
  private int range;


  public SlideMenu1(Context context) {
    this(context, null);
  }

  public SlideMenu1(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public SlideMenu1(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  protected void onFinishInflate() {
    ...
    setChildWidth(menuView, (int) (screenWidth * leftClosePercent));
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    closeDragRange = (int) (getMeasuredWidth() * leftClosePercent);
    openDragRange = (int) (getMeasuredWidth() * leftOpenPercent);
    range = openDragRange - closeDragRange;
    Logger.e("measuredWidth=" + getMeasuredWidth() + ",mainWidth=" + mainWidth + ",menuWidth=" + menuWidth);
  }

  @Override
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    menuView.layout(0, 0, closeDragRange, mainHeight);
    mainView.layout(closeDragRange, 0, screenWidth, mainHeight);
    Logger.e(TAG+"onLayout: 111111111111111111111111" );
  }

  @Override
  public void draw(Canvas canvas) {
    super.draw(canvas);
    Logger.e(TAG+ "draw: 22222222222222" );
  }
...
//        }
    ...
    public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
      ...
      if (changedView == mainView) {
        int newRight = menuView.getRight() + dx;
        newRight = clampRight(newRight);
        menuView.layout(0,0,newRight+menuWidth,menuView.getBottom());
        setChildWidth(menuView, (int) (screenWidth * leftClosePercent));
        Log.e(TAG, "onViewPositionChanged:newRight= "+newRight+",menuWidth="+menuWidth );
...
//      }
      ...
      //1.计算mainView滑动的百分比
      float fraction =  Math.abs(mainView.getLeft())/ getMeasuredWidth();
//      setChildWidth(menuView, (int) (screenWidth * leftClosePercent));
//            Log.e("tag",fraction+"");
...
//            executeAnim(fraction);
    ...
    public void onViewReleased(View releasedChild, float xvel, float yvel) {
      ...
      Logger.e(TAG + " onViewReleased: ");
      //判断mainView的left是否是大于一半
      if (mainView.getLeft() >closeDragRange+range/2 ) {
        //滑到右边
        viewDragHelper.smoothSlideViewTo(mainView, openDragRange, 0);
        //刷新操作
        ViewCompat.postInvalidateOnAnimation(SlideMenu1.this);

      } else {
        //滑到左边
        viewDragHelper.smoothSlideViewTo(mainView, closeDragRange, 0);
        //刷新操作
        ViewCompat.postInvalidateOnAnimation(SlideMenu1.this);
      }
    }
...
//            getBackground().setColorFilter((Integer) argbEvaluator.evaluate(fraction,Color.RED,Color.GREEN),PorterDuff.Mode.SRC_OVER);
  ...
  private int clampRight(int newRight) {
    if (newRight > openDragRange) {
      newRight = openDragRange;
    } else if (newRight < closeDragRange) {
      newRight = closeDragRange;
    }
    return newRight;
  ...
  private int clampMainLeft(int left) {
    if (left > openDragRange) {
      left = openDragRange;
    } else if (left < closeDragRange) {
      left = closeDragRange;
    }
    return left;
  }
