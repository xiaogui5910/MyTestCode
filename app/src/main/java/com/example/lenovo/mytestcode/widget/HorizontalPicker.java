...
public class HorizontalPicker extends View {
  ...
  private HorizontalPickerOnPageChangeListener pageChangeListener;
  ...
  public HorizontalPicker(Context context, AttributeSet attrs, int defStyle) {
    ...
    int defaultSelected = this.defaultSelected;
    ...
    try {
      ...
      Log.e(TAG, "HorizontalPicker: dividerSize="+dividerSize );
      sideItems = a.getInt(R.styleable.HorizontalPicker_sideItems, sideItems);
      selectedItem = a.getInt(R.styleable.HorizontalPicker_defaultSelected, defaultSelected);
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    //触摸picker
    pageChangeListener.setTouchPicker(false);
  ...
  public int getItemWithPadding() {
//      int itemWithPadding = (int) (itemWidth + dividerSize);
    return (int) (itemWidth + dividerSize);
  }
  ...
  public void setupWithViewPager(@Nullable ViewPager viewPager) {
    ...
    if (viewPager!=null){
      if (pageChangeListener == null) {
        pageChangeListener = new HorizontalPicker.HorizontalPickerOnPageChangeListener(this);
      }
  ...
  public void setDefaultSelectedItem(int defaultSelectedItem){
...
//            postInvalidateOnAnimation(); // TODO
  ...
  private void calculateItemSize(int w, int h) {
    ...
    Log.e(TAG, "calculateItemSize: selectedItem="+selectedItem );
  ...
  public static class HorizontalPickerOnPageChangeListener implements ViewPager.OnPageChangeListener {

    /**
     * 手指拖动viewPager方向
     */
    private  boolean isPullToLeft =true;
    private  boolean isScrolling = false;
    private  int lastValue = -1;
    /**
     * 是否触摸了水平选择栏，默认为true
     */
    private  boolean isTouchPicker = true;
    private final WeakReference<HorizontalPicker> mHorizontalPickerRef;

    public HorizontalPickerOnPageChangeListener(HorizontalPicker horizontalPicker) {
      mHorizontalPickerRef = new WeakReference<>(horizontalPicker);
    }
    ...
    public void onPageSelected(int position) {
      final HorizontalPicker picker = mHorizontalPickerRef.get();
      if (picker!=null &&picker.getSelectedItem()!=position &&position<picker.getValues().length){
        if (isTouchPicker){
          picker.smoothScrollBy(isPullToLeft ? 1 : -1);
        }
      }
    }
    ...
    public boolean isTouchPicker() {
      return isTouchPicker;
    }

    public void setTouchPicker(boolean touchPicker) {
      isTouchPicker = touchPicker;
    }
  }
}
